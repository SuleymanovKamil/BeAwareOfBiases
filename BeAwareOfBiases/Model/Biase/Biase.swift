...
struct Biase: Identifiable, Hashable {
    ...
    static var biases: [Biase] = [
        ...
        Biase(title: "Проклятие знания", subtitle: "Когда мы что-то знаем, то уверены, что все остальные тоже это знают.", description: "Учитель уже обладает знанием, которое он пытается передать, но способ передачи знания может не подходить тем, у кого этого знания нет.", image: "book.closed", type: [.memory,.belief, .social, .study], url: "https://ru.wikipedia.org/wiki/%D0%9F%D1%80%D0%BE%D0%BA%D0%BB%D1%8F%D1%82%D0%B8%D0%B5_%D0%B7%D0%BD%D0%B0%D0%BD%D0%B8%D1%8F"),
